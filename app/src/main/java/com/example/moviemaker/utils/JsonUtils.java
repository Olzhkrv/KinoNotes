...
public class JsonUtils {
  ...
  private static final String LOGO_POSTER_SIZE = "w154";
  ...
  //Передаем объект JSONObject. getMoviesFromJSON возвращает массив с фильмами
  public static ArrayList<Movie> getMoviesFromJSON(JSONObject jsonObject) {
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.length(); i++) {
        ...
        String logoPosterPath = BASE_POSTER_URL + LOGO_POSTER_SIZE + objectMovie.getString(KEY_POSTER_PATH);
        ...
        Movie movie = new Movie(id, voteCount, title, originalTitle, overview, posterPath,
                logoPosterPath, backdropPath, bigPosterPath, voteAverage, releaseDate);
        result.add(movie);
      }
  ...
  //Передаем объект JSONObject. getReviewFromJSON возвращает массив с отзывами
  public static ArrayList<Review> getReviewFromJSON(JSONObject object) {
    ArrayList<Review> result = new ArrayList<>();
    if(object == null) {
      return result;
    }
    try {
      JSONArray jsonArray = object.getJSONArray(KEY_RESULTS);
      for (int i = 0; i < jsonArray.length(); i++) {
  ...
  //Передаем объект JSONObject. getReviewFromJSON возвращает массив с видео
  public static ArrayList<Trailer> getVideoFromJSON(JSONObject object) {
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.length(); i++) {
        ...
        String key = BASE_YOUTUBE_URL + objectTrailers.getString(KEY_KEY);
